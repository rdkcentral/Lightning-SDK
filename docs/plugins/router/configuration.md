...
If you want to display a **Splash** or **Loading** screen while the Router is booting (and *before* the actual routing process kicks in),
...
You can also redirect the `navigate` by
returning a *String*. The Router will then try to navigate to the provided hash.
...
you set `reuseInstance: false`. This [setting](settings.md#reuseinstance) is also globally available.
