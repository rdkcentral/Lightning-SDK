...
export default class WebAudio extends BaseAudio {
    ...
    try {
      if (this._playing) {
        ...
        if (this._loop && playedTime > this._duration) {
          ...
          offset = parseFloat(
            (playedTime + this._currentOffset - numOfLoops * this._duration).toFixed(2)
          )
        } else {
  ...
  compress(compressorParams) {
    ...
    const compressor = this._nodes.has('compressor')
      ? this._nodes.get('compressor')
      : this._audioContext.createDynamicsCompressor()
  ...
  filter(filterParams) {
    ...
    const filter = this._nodes.has(key)
      ? this._nodes.get(key)
  ...
  IIRFilter(feedForward, feedBack) {
    if (!validateCoeff('feedForward', feedForward) || !validateCoeff('feedBack', feedBack)) {
      return this
    }
  ...
  distortion(amount, oversample) {
    if (this._validate('distortion amount', amount, [0, 1])) {
      const distortionNode = this._nodes.has('distortion')
        ? this._nodes.get('distortion')
        : this._audioContext.createWaveShaper()
  ...
  panner(pannerParams) {
    ...
    const key = 'panner'
    const pannerNode = this._nodes.has(key)
      ? this._nodes.get(key)
      : this._audioContext.createPanner()
  ...
  stereoPanner(pan) {
    if (this._validate('pan', pan, [-1, 1])) {
      const stereoPannerNode = this._nodes.has('stereoPanner')
        ? this._nodes.get('stereoPanner')
        : this._audioContext.createStereoPanner()
